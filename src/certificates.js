...
const generateieeeExecom2023 = async (name, certificate, post,no) => {
      ...
      if (certificate === "IEEE EXECOM 2023") {
        certificate1 = await fetch(
          "./certificates/cert-data/ieeeexecom2023/data.json"
        ).then((res) => {
          return res.json();
        });
        ...
        exBytes1 = await fetch("./certificates/cert-pdf/ieeeexecom2023/a.pdf").then(
          (res) => {
            return res.arrayBuffer();
          }
        );
        ...
        anchor.download = "ieee_excom_2023_certificate.pdf";
        anchor.appendChild(elem);
        main.appendChild(anchor);
